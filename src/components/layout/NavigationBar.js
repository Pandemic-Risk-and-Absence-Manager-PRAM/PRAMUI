...
const NavigationBar = ({ isOpen, toggleNavigationBar }) => {
    ...
    switch (dashboardType) {
      case 'employee':
        return [
          ...
          { name: "Request Time Off", icon: <PiAirplaneTakeoffThin />, link: `/dashboard/${dashboardType}/request-time-off` },
  ...
  return (
    <div 
      className={`fixed top-0 left-0 z-30 bg-[#1F3557] dark:bg-gray-800 p-5 pt-[80px] transition-all duration-300 ${
        isOpen ? "w-64" : "w-20"
      } h-screen flex flex-col overflow-y-hidden max-h-screen`}
    >
        {/* Search Bar */}
        {isOpen && (
